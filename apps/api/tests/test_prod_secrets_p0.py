...
def _prod_settings(**over) -> Settings:
    base = dict(
        ...
        # DM Tool's own SMTP transport must be wired (TLS-safe) in production so
        # verification / reset links are actually delivered (not just logged).
        smtp_host="mail.dmtool.internal",
        smtp_from="DM Tool <no-reply@dm.example>",
        smtp_tls="starttls",
    )
    base.update(over)
...
def test_prod_requires_smtp_delivery():
    # Production must fail closed when DM Tool's own SMTP transport isn't wired,
    # otherwise verification/reset links only reach the logs.
    with pytest.raises(SystemExit) as exc:
        validate_production_secrets(_prod_settings(smtp_host=""))
    assert "SMTP" in str(exc.value)


def test_prod_requires_tls_smtp():
    # TLS must not be silently downgraded in production.
    with pytest.raises(SystemExit) as exc:
        validate_production_secrets(_prod_settings(smtp_tls="none"))
