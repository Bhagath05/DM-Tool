...
export function RecentList({
  ...
  items: GeneratedVisual[];
  activeId?: string;
  onSelect: (item: GeneratedVisual) => void;
}) {
  return (
      ...
      <CardContent className="space-y-1">
        {items.length === 0 && (
          <p className="text-xs text-muted-foreground">
            Your generated creatives will show up here.
          </p>
        )}
        {items.map((item) => {
          const active = item.id === activeId;
          const thumb = item.thumbnail_url ?? item.primary_signed_url;
          return (
            <button
              key={item.id}
              type="button"
